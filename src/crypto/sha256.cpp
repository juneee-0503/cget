...
public:
    ...
    std::string final() {
        ...
        for (int i = 7; i >= 0; --i) {
            buffer_[bufferSize_++] = static_cast<unsigned char>((bitLength_ >> (i * 8)) & 0xffU);
        }
        transform(buffer_.data());
...
std::string sha256File(const std::filesystem::path& path) {
    ...
    while (input) {
        ...
        if (read > 0) {
            sha.update(reinterpret_cast<const unsigned char*>(buffer.data()), static_cast<std::size_t>(read));
        }
    }
    return sha.final();
}
...
std::string lowercaseHex(std::string value) {
    for (char& c : value) {
        if (c >= 'A' && c <= 'F') {
